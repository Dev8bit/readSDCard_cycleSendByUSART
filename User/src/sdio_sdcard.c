...
/* Private variables ---------------------------------------------------------*/
static uint32_t CardType =  SDIO_STD_CAPACITY_SD_CARD_V1_1;	//存储卡的类型，存储卡一般有两种协议，2G以下的卡一般是1.1协议
...
void NVIC_Configuration(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;  //不确定是否应该开启sdio中断，usb模式会接管sd卡，但stm32的文件系统需要sdio中断
...
/**************************************************************
*	功能 ：配置SDIO数据接收为DMA方式（DMA2通道4）
*	参数：BufferDST：用于装载数据的变量指针
	      BufferSize：	缓冲区大小
**************************************************************/
...
/**********************************************************
*	功能：判断SD卡是否在进行数据传输
*	返回值：SD_TRANSFER_OK	没有进行数据传输，或者传输已完成
*			SD_TRANSFER_BUSY 正在进行传输
**********************************************************/
...
/*************************************************
*	功能：判断存储卡的当前状态
*	返回值： SD Card Error or SD Card Current State.
*************************************************/
...
/*********************************************************
*	功能：获取SD卡的具体信息
*	参数：-cardinfo 指向SD_CardInfo结构体的指针
*         这个结构里面包含了SD卡的具体信息
*	返回值 ：-SD_Error SD卡错误代码
*         成功时则为 SD_OK
*********************************************************/
...
/**************************************************************************
*	功能：配置卡的数据宽度(但得看卡是否支持)
*	参数：-WideMode 指定SD卡的数据线宽
*         具体可配置如下
*         @arg SDIO_BusWide_8b: 8-bit data transfer (Only for MMC)
*         @arg SDIO_BusWide_4b: 4-bit data transfer
...
/*****************************************************************
*	功能：利用cmd7，选择卡相对地址为addr的卡，取消选择其它卡
*   		如果addr = 0,则取消选择所有的卡
*	参数：-addr 选择卡的地址
*	返回值：-SD_Error SD卡错误代码
*         成功时则为 SD_OK
...
/***************************************************************
*	功能：向SD卡中读出多块数据
*	参数：readbuff 保存读取到的数据
*		  ReadAddr 指定读取数据的起始地址
*		  BlockSize 数据块大小，SD卡必须为512
*		  NumberOfBlocks 指定带读取的数据块的数量	
***************************************************************/
...
/**
  ...
  */
/***********************************************************************
*	功能：用DMA方式向SD卡中写入一块数据
*	参数：writebuff 指向待写入的数据
*		  WriteAddr 指定写入的地址即SD卡存储区域中的地址
*		  BlockSize 数据块大小，SD卡的数据块大小必须为512
***********************************************************************/
...
/*****************************************************
*	功能：检测SD卡是不是正在进行内部读写操作
*	参数 ：用来装载SD state状态的指针
*	返回值 ：SD错误类型
*****************************************************/
static SD_Error IsCardProgramming(uint8_t *pstatus)
{
